...
void RenderSystem::render(const int& x, const int& y)
{
	for (const auto& e : m_entities)
	{
		...
		SDL_RenderCopyExF(Game::renderer, render.texture, &render.src_rect, &SDL_FRect({ position.pos.x - x, position.pos.y - y, size.size.x, size.size.y }), move.rotation, NULL, SDL_FLIP_NONE);
	}
}
